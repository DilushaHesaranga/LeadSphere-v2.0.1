...
The repository also contains an Expo/React Native TypeScript mobile application in `mobile/`. It supports Sales Executives, Marketing Executives, Sales Managers, and Delivery Managers with the same authentication lifecycle, daily dashboard, lead/customer Ticket work, contacts and notes, full Follow Up workflow, and sales pipeline backed by the web application's trusted contracts. Setup, build, security, offline behavior, and Firebase App Distribution instructions are in `docs/MOBILE.md`.
