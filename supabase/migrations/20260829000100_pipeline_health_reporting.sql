-- Keep Pipeline Health aligned with the Active Pipeline KPI and its drill-down.
-- The focused RPC reports only active Tickets in canonical open stages, while
-- preserving empty open stages so users can understand the whole pipeline.
...
begin
  ...
  with current_scope as materialized (
    ...
    left join current_scope scoped
      ...
      and scoped.stage_category = 'open'
  ...
  select
    ...
    count(*) filter (where grouped.total > 0 and grouped.average_age_days >= 14)
  into stage_series, active_total, occupied_total, attention_total
  from grouped;
